...
1. Build the cointainer
```bash
docker build -t ros2-livekit-bridge-dev --build-arg WS_ROS=livekit_ws .
```

2. Run the container
```bash
 docker run -d --name livekit-dev --privileged --init --ipc=host --network=host -v "$PWD":/livekit_ws -w /livekit_ws ros2-livekit-bridge-dev sleep infinity
```

3 Exec into the container
```bash
docker exec -it livekit-dev bash
```
