...
namespace ros2_livekit_bridge::cli::interface_show {
...
inline std::optional<std::string> interfacePath(const std::string& type) {
  ...
  try {
    const auto share_directory = ament_index_cpp::get_package_share_directory(package_name);
    const auto extension = "." + interface_kind;
    return share_directory + "/" + interface_kind + "/" + interface_name + extension;
...
inline std::string stripTrailingComment(const std::string& line) {
  ...
  if (comment_start == std::string::npos) {
    return cli::rightTrim(line);
  }
  return cli::rightTrim(line.substr(0, comment_start));
}
...
inline std::optional<std::string> nestedInterfaceTypeFromLine(const std::string& package_name,
                                                              const std::string& line) {
  const auto without_comment = stripTrailingComment(line);
  const auto trimmed = cli::leftTrim(without_comment);
  ...
  if (type_token.empty() || name_token.empty() || name_token.find('=') != std::string::npos) {
    return std::nullopt;
  }
  ...
  if (parts.size() == 1) {
    if (!type_token.empty() && std::isupper(static_cast<unsigned char>(type_token.front()))) {
...
inline void appendRenderedInterfaceLine(std::ostringstream& output, const std::string& line, bool show_comments,
                                        int indent_level) {
  if (show_comments) {
    if (!line.empty()) {
  ...
  const auto trimmed = cli::leftTrim(line);
