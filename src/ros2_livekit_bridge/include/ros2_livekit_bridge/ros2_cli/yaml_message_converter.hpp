...
/// @param interface_type ROS interface type, such as `std_msgs/msg/String`.
...
/// @param error Set to a description when @p node is not an integer or is out of range.
  ...
  std::string & error)
{
  ...
  try {
    if constexpr (std::is_signed_v<T>) {
      ...
      {
        error = "field '" + path +
          "' must be an integer: integer is out of range";
        return std::nullopt;
      }
...
/// @brief Convert a YAML node to a floating point type, enforcing its magnitude.
/// @tparam T Target floating point type.
/// @param node YAML scalar node.
/// @param path Field path used in error messages.
/// @param error Set to a description when @p node is not numeric or is out of range.
/// @return The parsed value as @p T, or `std::nullopt` on failure.
template<typename T>
...
/// @param path Field path used in error messages.
/// @param error Set to a description when @p node is not a string or exceeds @p upper_bound.
...
/// @param path Field path used in error messages.
/// @param error Set to a description when @p node is not a string or exceeds @p upper_bound.
/// @return The parsed string widened code unit by code unit, or `std::nullopt` on failure.
...
/// @details A single-character scalar is taken verbatim; anything else is parsed
///   as an integer code point.
/// @param node YAML scalar node.
/// @param path Field path used in error messages.
/// @param error Set to a description when @p node is neither a character nor integer.
