...
private:
  ...
  /// @param entries Configured service forwarding entries.
  /// @return True on success, false when the service forwarder could not be initialized.
  bool initializeServiceForwarder(std::vector<ServiceForwarder::ServiceForwarderEntry> entries);
  ...
  //! @brief Forwards ROS service calls to LiveKit RPC and vice versa.
