...
#include "diagnostics/connection_health.hpp"
  ...
  std::visit(
    [&accumulator](const auto & typed_stats) {
      ...
      if constexpr (std::is_same_v<StatsType, livekit::RtcTransportStats>) {
        ...
        StatsType, livekit::RtcCandidatePairStats>)
      {
        accumulator.candidate_pairs.push_back(
          {typed_stats.rtc.id, typed_stats.rtc.timestamp_ms,
            typed_stats.candidate_pair});
      } else if constexpr (std::is_same_v<
        StatsType, livekit::RtcInboundRtpStats>)
      {
        accumulator.packets_lost += typed_stats.received.packets_lost;
        updateMaxJitter(accumulator, typed_stats.received.jitter);
      } else if constexpr (std::is_same_v<
        StatsType, livekit::RtcRemoteInboundRtpStats>)
      {
        accumulator.packets_lost += typed_stats.received.packets_lost;
        updateMaxJitter(accumulator, typed_stats.received.jitter);
      } else if constexpr (std::is_same_v<
        StatsType, livekit::RtcDataChannelStats>)
      {
...
const TransportSnapshot * selectTransport(
  const RtcStatsAccumulator & accumulator)
{
  const auto selected_transport = std::find_if(
    accumulator.transports.begin(),
    accumulator.transports.end(),
...
const CandidatePairSnapshot * findCandidatePairById(
  const RtcStatsAccumulator & accumulator,
  const std::string & id)
{
  const auto selected_pair = std::find_if(
    accumulator.candidate_pairs.begin(),
    accumulator.candidate_pairs.end(),
    [&id](const auto & candidate_pair) {
      return candidate_pair.id == id;
    });
...
const CandidatePairSnapshot * selectCandidatePair(
  const RtcStatsAccumulator & accumulator,
  ...
  const CandidatePairSnapshot & candidate_pair)
{
  ...
  if (!summary.bytes_sent.has_value() ||
    !summary.bytes_received.has_value())
  {
    ...
    state.previous_traffic->candidate_pair_id == candidate_pair.id)
  {
    ...
    summary.send_bitrate_bps = computeBitrate(
      state.previous_traffic->bytes_sent,
      *summary.bytes_sent,
      elapsed_ms);
    summary.receive_bitrate_bps = computeBitrate(
      state.previous_traffic->bytes_received,
  ...
  const ConnectionHealthRtcSummary & summary)
{
  ...
  addOptionalField(
    status,
    "rtc.transport.candidate_pair_state",
    summary.candidate_pair_state);
  addOptionalField(
    status,
    "rtc.transport.current_round_trip_time_ms",
    summary.current_round_trip_time_ms);
  addOptionalField(
    status,
  ...
  addOptionalField(
    status,
    "rtc.traffic.bytes_received",
    summary.bytes_received);
  addOptionalField(
    status,
    "rtc.traffic.send_bitrate_bps",
    summary.send_bitrate_bps);
  addOptionalField(
    status,
    "rtc.traffic.receive_bitrate_bps",
...
: state_{
    ConnectionHealthStateKind::Disconnected,
    std::move(room_name),
    0,
    0,
    {},
    std::nullopt},
  updater_(node, kDefaultDiagnosticPeriodSec)
{
...
void ConnectionHealthDiagnostics::onRoomUpdated(
  livekit::Room & room,
  const livekit::RoomUpdatedEvent &)
{
  updatePeerCount(room);
}
