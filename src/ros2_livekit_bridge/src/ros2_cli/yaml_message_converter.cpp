...
{
  return static_cast<void *>(static_cast<std::uint8_t *>(message) +
         member.offset_);
}
...
template<typename T>
bool assignScalarValue(
  const MessageMember & member, const YAML::Node & node,
  void *destination, const std::string & path,
  std::string & error)
{
  (void)member;
...
template<>
bool assignScalarValue<std::string>(
  const MessageMember & member,
  const YAML::Node & node, void *destination,
  const std::string & path,
  std::string & error)
{
  auto value =
    detail::checkedString(node, member.string_upper_bound_, path, error);
  if (!value) {
    return false;
  }
...
template<>
bool assignScalarValue<std::u16string>(
  const MessageMember & member,
...
{
  auto value =
    detail::checkedU16String(node, member.string_upper_bound_, path, error);
  if (!value) {
    return false;
  }
...
template<>
bool assignScalarValue<char16_t>(
  const MessageMember & member,
  const YAML::Node & node, void *destination,
  const std::string & path, std::string & error)
{
...
template<typename T>
bool assignArrayScalarValue(
  const MessageMember & member, const YAML::Node & node,
  void *sequence, std::size_t index,
  const std::string & path, std::string & error)
{
  T value{};
...
bool assignField(
  const MessageMember & member, const YAML::Node & node,
  void *field_memory, const std::string & path,
  std::string & error);

bool assignMessage(
  const YAML::Node & node, void *message,
  const MessageMembers & members, const std::string & path,
  std::string & error)
{
  if (!node.IsMap()) {
  ...
  for (const auto & entry : node) {
    ...
    if (member == nullptr) {
      error = "field '" + path + "." + field_name +
        "' is not defined by message type '" +
        members.message_namespace_ + "::" + members.message_name_ + "'";
      return false;
    }
    if (!assignField(*member, entry.second, memberMemory(message, *member),
                     path + "." + field_name, error))
    {
      return false;
    }
  }
...
bool isResizableArray(const MessageMember & member)
{
  return member.resize_function != nullptr;
}

bool validateAndResizeArray(
  const MessageMember & member, const YAML::Node & node,
  void *field_memory, const std::string & path,
  std::string & error)
{
  ...
  if (!isResizableArray(member)) {
    if (requested_size != member.array_size_) {
      error = "field '" + path + "' must contain exactly " +
        std::to_string(member.array_size_) + " entries";
      return false;
    }
    return true;
  }
  ...
  if (member.is_upper_bound_ && requested_size > member.array_size_) {
    error = "field '" + path + "' exceeds sequence upper bound " +
      std::to_string(member.array_size_);
    return false;
  }
  if (requested_size > kMaxResizableSequenceLength) {
    error = "field '" + path + "' exceeds maximum sequence length " +
      std::to_string(kMaxResizableSequenceLength);
    return false;
  }
  ...
  for (std::size_t index = 0; index < node.size(); ++index) {
    ...
    switch (member.type_id_) {
      case introspection::ROS_TYPE_FLOAT:
        ok = assignArrayScalarValue<float>(member, node[index], field_memory,
                                         index, element_path, error);
        break;
      case introspection::ROS_TYPE_DOUBLE:
        ok = assignArrayScalarValue<double>(member, node[index], field_memory,
                                          index, element_path, error);
        break;
      case introspection::ROS_TYPE_LONG_DOUBLE:
        ok = assignArrayScalarValue<long double>(
          member, node[index], field_memory, index, element_path, error);
        break;
      case introspection::ROS_TYPE_CHAR:
        ok = assignArrayScalarValue<char>(member, node[index], field_memory,
                                        index, element_path, error);
        break;
      case introspection::ROS_TYPE_WCHAR:
        ok = assignArrayScalarValue<char16_t>(member, node[index], field_memory,
                                            index, element_path, error);
        break;
      case introspection::ROS_TYPE_BOOLEAN:
        ok = assignArrayScalarValue<bool>(member, node[index], field_memory,
                                        index, element_path, error);
        break;
      case introspection::ROS_TYPE_OCTET:
      case introspection::ROS_TYPE_UINT8:
        ok = assignArrayScalarValue<std::uint8_t>(
          member, node[index], field_memory, index, element_path, error);
        break;
      case introspection::ROS_TYPE_INT8:
        ok = assignArrayScalarValue<std::int8_t>(
          member, node[index], field_memory, index, element_path, error);
        break;
      case introspection::ROS_TYPE_UINT16:
        ok = assignArrayScalarValue<std::uint16_t>(
          member, node[index], field_memory, index, element_path, error);
        break;
      case introspection::ROS_TYPE_INT16:
        ok = assignArrayScalarValue<std::int16_t>(
          member, node[index], field_memory, index, element_path, error);
        break;
      case introspection::ROS_TYPE_UINT32:
        ok = assignArrayScalarValue<std::uint32_t>(
          member, node[index], field_memory, index, element_path, error);
        break;
      case introspection::ROS_TYPE_INT32:
        ok = assignArrayScalarValue<std::int32_t>(
          member, node[index], field_memory, index, element_path, error);
        break;
      case introspection::ROS_TYPE_UINT64:
        ok = assignArrayScalarValue<std::uint64_t>(
          member, node[index], field_memory, index, element_path, error);
        break;
      case introspection::ROS_TYPE_INT64:
        ok = assignArrayScalarValue<std::int64_t>(
          member, node[index], field_memory, index, element_path, error);
        break;
      case introspection::ROS_TYPE_STRING:
        ok = assignArrayScalarValue<std::string>(
          member, node[index], field_memory, index, element_path, error);
        break;
      case introspection::ROS_TYPE_WSTRING:
        ok = assignArrayScalarValue<std::u16string>(
          member, node[index], field_memory, index, element_path, error);
        break;
      default:
        error = "field '" + element_path + "' has unsupported ROS type id " +
          std::to_string(member.type_id_);
        return false;
    }
    if (!ok) {
      return false;
  ...
  std::string & error)
{
  ...
  switch (member.type_id_) {
    ...
    case introspection::ROS_TYPE_LONG_DOUBLE:
      return assignScalarValue<long double>(member, node, field_memory, path,
                                          error);
    case introspection::ROS_TYPE_CHAR:
    ...
    case introspection::ROS_TYPE_OCTET:
    case introspection::ROS_TYPE_UINT8:
      return assignScalarValue<std::uint8_t>(member, node, field_memory, path,
                                           error);
    case introspection::ROS_TYPE_INT8:
      return assignScalarValue<std::int8_t>(member, node, field_memory, path,
                                          error);
    case introspection::ROS_TYPE_UINT16:
      return assignScalarValue<std::uint16_t>(member, node, field_memory, path,
                                            error);
    case introspection::ROS_TYPE_INT16:
      return assignScalarValue<std::int16_t>(member, node, field_memory, path,
                                           error);
    case introspection::ROS_TYPE_UINT32:
      return assignScalarValue<std::uint32_t>(member, node, field_memory, path,
                                            error);
    case introspection::ROS_TYPE_INT32:
      return assignScalarValue<std::int32_t>(member, node, field_memory, path,
                                           error);
    case introspection::ROS_TYPE_UINT64:
      return assignScalarValue<std::uint64_t>(member, node, field_memory, path,
                                            error);
    case introspection::ROS_TYPE_INT64:
      return assignScalarValue<std::int64_t>(member, node, field_memory, path,
                                           error);
    case introspection::ROS_TYPE_STRING:
      return assignScalarValue<std::string>(member, node, field_memory, path,
                                          error);
    case introspection::ROS_TYPE_WSTRING:
      return assignScalarValue<std::u16string>(member, node, field_memory, path,
                                             error);
    ...
    default:
      error = "field '" + path + "' has unsupported ROS type id " +
        std::to_string(member.type_id_);
      return false;
  }
}
  ...
  const std::string & payload, std::string & error)
{
  ...
  if (payload.size() > kMaxYamlPayloadBytes) {
    error = "payload exceeds maximum size of " +
      std::to_string(kMaxYamlPayloadBytes) + " bytes";
    return std::nullopt;
  }
  ...
  try {
    ...
    const auto *members = messageMembersFromTypeSupport(
        introspection_type_support, msg_type, error);
    if (members == nullptr) {
      return std::nullopt;
    }
