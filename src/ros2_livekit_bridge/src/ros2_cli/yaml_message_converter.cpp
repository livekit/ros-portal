...
const MessageMembers * messageMembersFromTypeSupport(
  const rosidl_message_type_support_t * type_support,
  const std::string & interface_type,
  std::string & error)
{
  if (type_support == nullptr || type_support->data == nullptr) {
    error = "type support for '" + interface_type +
...
void * memberMemory(void * message, const MessageMember & member)
{
  return static_cast<void *>(
    static_cast<std::uint8_t *>(message) + member.offset_);
}
  ...
  std::string & error)
{
  ...
  for (const auto & entry : node) {
    ...
    if (!assignField(
        *member, entry.second, memberMemory(message, *member),
  ...
  for (std::size_t index = 0; index < node.size(); ++index) {
    ...
    switch (member.type_id_) {
      ...
      case introspection::ROS_TYPE_CHAR:
        ok = assignArrayScalarValue<char>(
          member, node[index], field_memory, index, element_path, error);
        break;
      case introspection::ROS_TYPE_WCHAR:
        ok = assignArrayScalarValue<char16_t>(
          member, node[index], field_memory, index, element_path, error);
        break;
      case introspection::ROS_TYPE_BOOLEAN:
        ok = assignArrayScalarValue<bool>(
          member, node[index], field_memory, index, element_path, error);
        break;
  ...
  std::string & error)
{
  ...
  switch (member.type_id_) {
    ...
    case introspection::ROS_TYPE_LONG_DOUBLE:
      return assignScalarValue<long double>(
        member, node, field_memory, path, error);
    case introspection::ROS_TYPE_CHAR:
      return assignScalarValue<char>(member, node, field_memory, path, error);
    case introspection::ROS_TYPE_WCHAR:
      return assignScalarValue<char16_t>(
        member, node, field_memory, path, error);
    case introspection::ROS_TYPE_BOOLEAN:
      return assignScalarValue<bool>(member, node, field_memory, path, error);
    case introspection::ROS_TYPE_OCTET:
    case introspection::ROS_TYPE_UINT8:
      return assignScalarValue<std::uint8_t>(
        member, node, field_memory, path, error);
    case introspection::ROS_TYPE_INT8:
      return assignScalarValue<std::int8_t>(
        member, node, field_memory, path, error);
    case introspection::ROS_TYPE_UINT16:
      return assignScalarValue<std::uint16_t>(
        member, node, field_memory, path, error);
    case introspection::ROS_TYPE_INT16:
      return assignScalarValue<std::int16_t>(
        member, node, field_memory, path, error);
    case introspection::ROS_TYPE_UINT32:
      return assignScalarValue<std::uint32_t>(
        member, node, field_memory, path, error);
    case introspection::ROS_TYPE_INT32:
      return assignScalarValue<std::int32_t>(
        member, node, field_memory, path, error);
    case introspection::ROS_TYPE_UINT64:
      return assignScalarValue<std::uint64_t>(
        member, node, field_memory, path, error);
    case introspection::ROS_TYPE_INT64:
      return assignScalarValue<std::int64_t>(
        member, node, field_memory, path, error);
    case introspection::ROS_TYPE_STRING:
      return assignScalarValue<std::string>(
        member, node, field_memory, path, error);
    case introspection::ROS_TYPE_WSTRING:
      return assignScalarValue<std::u16string>(
        member, node, field_memory, path, error);
...
std::optional<rclcpp::SerializedMessage> serializedMessageFromYaml(
  const std::string & interface_type,
  ...
  std::string & error)
{
  ...
  try {
    auto introspection_library = rclcpp::get_typesupport_library(
      interface_type, introspection::typesupport_identifier);
    ...
    const auto * members = messageMembersFromTypeSupport(
      introspection_type_support, interface_type, error);
    if (members == nullptr) {
      return std::nullopt;
    }

    DynamicMessage message(*members);
    if (!assignMessage(root, message.get(), *members, interface_type, error)) {
      return std::nullopt;
    }

    auto serialization_library = rclcpp::get_typesupport_library(
      interface_type, rosidl_typesupport_cpp::typesupport_identifier);
