...
Ros2CliManager::Ros2CliManager(NodeInterfaces node_interfaces, rclcpp::CallbackGroup::SharedPtr callback_group,
  ...
  if (!node_interfaces_.node_base || !node_interfaces_.node_services || !node_interfaces_.node_graph ||
    ...
    throw std::invalid_argument("Ros2CliManager requires fully populated NodeInterfaces");
  }
  ...
  if (!livekit_methods_.has_participant || !livekit_methods_.perform_rpc || !livekit_methods_.register_rpc_method ||
    ...
    throw std::invalid_argument("Ros2CliManager requires fully populated LivekitMethods");
  }
  ...
  topic_publisher_ = std::make_unique<ros2_cli::Ros2TopicPub>(node_interfaces_.node_topics, node_interfaces_.node_graph,
                                                              topic_publish_allowed_);
  service_caller_ = std::make_unique<ros2_cli::Ros2ServiceCall>(node_interfaces_.node_base, node_interfaces_.node_graph,
                                                                node_interfaces_.node_logging->get_logger());

  topic_list_service_ = rclcpp::create_service<Ros2TopicList>(
      node_interfaces_.node_base, node_interfaces_.node_services, ros2_cli::kTopicListServiceName,
      [this](const std::shared_ptr<Ros2TopicList::Request> request, std::shared_ptr<Ros2TopicList::Response> response) {
  ...
  topic_pub_service_ = rclcpp::create_service<Ros2TopicPubSrv>(
      node_interfaces_.node_base, node_interfaces_.node_services, ros2_cli::kTopicPubServiceName,
      [this](const std::shared_ptr<Ros2TopicPubSrv::Request> request,
             std::shared_ptr<Ros2TopicPubSrv::Response> response) { handleTopicPubRosService(request, response); },
      rclcpp::ServicesQoS(), callback_group);

  service_list_service_ = rclcpp::create_service<Ros2ServiceList>(
      node_interfaces_.node_base, node_interfaces_.node_services, ros2_cli::kServiceListServiceName,
      [this](const std::shared_ptr<Ros2ServiceList::Request> request,
             std::shared_ptr<Ros2ServiceList::Response> response) { handleServiceListRosService(request, response); },
      rclcpp::ServicesQoS(), callback_group);

  service_call_service_ = rclcpp::create_service<Ros2ServiceCallSrv>(
      node_interfaces_.node_base, node_interfaces_.node_services, ros2_cli::kServiceCallServiceName,
  ...
  interface_show_service_ = rclcpp::create_service<Ros2InterfaceShow>(
      node_interfaces_.node_base, node_interfaces_.node_services, ros2_cli::kInterfaceShowServiceName,
  ...
  RCLCPP_INFO(node_interfaces_.node_logging->get_logger(),
              "Registered ROS services:\n   - %s\n   - %s\n   - %s\n   - %s\n   - %s", ros2_cli::kTopicListServiceName,
              ros2_cli::kTopicPubServiceName, ros2_cli::kServiceListServiceName, ros2_cli::kServiceCallServiceName,
              ros2_cli::kInterfaceShowServiceName);
  RCLCPP_INFO(node_interfaces_.node_logging->get_logger(),
              "Registered LiveKit RPC methods:\n   - %s\n   - %s\n   - %s\n   - %s\n"
              "   - %s",
              ros2_cli::kTopicListRpcMethod, ros2_cli::kTopicPubRpcMethod, ros2_cli::kServiceListRpcMethod,
              ros2_cli::kServiceCallRpcMethod, ros2_cli::kInterfaceShowRpcMethod);
}

Ros2CliManager::Ros2CliManager(rclcpp::Node &node, rclcpp::CallbackGroup::SharedPtr callback_group,
                               LivekitMethods livekit_methods, TopicPublishAllowed topic_publish_allowed)
    : Ros2CliManager(
          NodeInterfaces{
...
void Ros2CliManager::handleTopicListRosService(const std::shared_ptr<Ros2TopicList::Request> request,
                                               std::shared_ptr<Ros2TopicList::Response> response) const {
  *response = callRemoteTopicList(*request);
}

void Ros2CliManager::handleTopicPubRosService(const std::shared_ptr<Ros2TopicPubSrv::Request> request,
                                              std::shared_ptr<Ros2TopicPubSrv::Response> response) const {
  *response = callRemoteTopicPub(*request);
}

void Ros2CliManager::handleServiceListRosService(const std::shared_ptr<Ros2ServiceList::Request> request,
                                                 std::shared_ptr<Ros2ServiceList::Response> response) const {
  *response = callRemoteServiceList(*request);
}

void Ros2CliManager::handleServiceCallRosService(const std::shared_ptr<Ros2ServiceCallSrv::Request> request,
                                                 std::shared_ptr<Ros2ServiceCallSrv::Response> response) const {
  *response = callRemoteServiceCall(*request);
}

void Ros2CliManager::handleInterfaceShowRosService(const std::shared_ptr<Ros2InterfaceShow::Request> request,
                                                   std::shared_ptr<Ros2InterfaceShow::Response> response) const {
  *response = callRemoteInterfaceShow(*request);
}

template <typename ResponseT>
ResponseT Ros2CliManager::performRemoteRpc(const std::string &participant_id, const char *rpc_method,
...
Ros2CliManager::Ros2TopicList::Response Ros2CliManager::callRemoteTopicList(
  ...
  return performRemoteRpc<Ros2TopicList::Response>(request.participant_id, ros2_cli::kTopicListRpcMethod, payload,
                                                   timeout_sec);
}

Ros2CliManager::Ros2TopicPubSrv::Response Ros2CliManager::callRemoteTopicPub(
    const Ros2TopicPubSrv::Request &request) const {
  if (request.participant_id.empty()) {
    return makeCliResponse<Ros2TopicPubSrv::Response>(false, "participant_id must be non-empty");
  }
  ...
  return performRemoteRpc<Ros2TopicPubSrv::Response>(request.participant_id, ros2_cli::kTopicPubRpcMethod, payload,
                                                     timeout_sec);
}
...
Ros2CliManager::Ros2ServiceList::Response Ros2CliManager::callRemoteServiceList(
  ...
  return performRemoteRpc<Ros2ServiceList::Response>(request.participant_id, ros2_cli::kServiceListRpcMethod, payload,
                                                     timeout_sec);
}

Ros2CliManager::Ros2ServiceCallSrv::Response Ros2CliManager::callRemoteServiceCall(
    const Ros2ServiceCallSrv::Request &request) const {
  if (request.participant_id.empty()) {
    return makeCliResponse<Ros2ServiceCallSrv::Response>(false, "participant_id must be non-empty");
  }

  if (request.service.empty()) {
    return makeCliResponse<Ros2ServiceCallSrv::Response>(false, "service must be non-empty");
  }
  ...
  if (request.payload.empty()) {
    return makeCliResponse<Ros2ServiceCallSrv::Response>(false, "payload must be non-empty");
  }
  ...
  return performRemoteRpc<Ros2ServiceCallSrv::Response>(request.participant_id, ros2_cli::kServiceCallRpcMethod,
                                                        payload, rpc_timeout_sec);
}

Ros2CliManager::Ros2InterfaceShow::Response Ros2CliManager::callRemoteInterfaceShow(
  ...
  if (request.all_comments && request.no_comments) {
    return makeCliResponse<Ros2InterfaceShow::Response>(false, "all_comments and no_comments are mutually exclusive");
  }
  ...
  return performRemoteRpc<Ros2InterfaceShow::Response>(request.participant_id, ros2_cli::kInterfaceShowRpcMethod,
                                                       payload, timeout_sec);
}
...
std::string Ros2CliManager::handleTopicListRpc(const std::string &payload) const {
  ...
  try {
    const auto output = ros2_cli::formatTopicList(
        ros2_cli::collectTopicInfo(*node_interfaces_.node_graph, options.value()), options.value());
...
std::string Ros2CliManager::handleTopicPubRpc(const std::string &payload) const {
  const auto options = topicPubOptionsFromJson(payload);
  if (!options) {
    RCLCPP_ERROR(node_interfaces_.node_logging->get_logger(), "Failed to handle LiveKit RPC '%s': %s",
...
std::string Ros2CliManager::handleServiceListRpc(const std::string &payload) const {
  ...
  try {
    const auto output = ros2_cli::formatServiceList(
        ros2_cli::collectServiceInfo(*node_interfaces_.node_graph, options.value()), options.value());
    return cliResponseToJson(true, "", output);
  } catch (const std::exception &error) {
    RCLCPP_ERROR(node_interfaces_.node_logging->get_logger(), "Failed to handle LiveKit RPC '%s': %s",
...
std::string Ros2CliManager::handleServiceCallRpc(const std::string &payload) const {
...
std::string Ros2CliManager::handleInterfaceShowRpc(const std::string &payload) const {
...
bool Ros2CliManager::isHiddenTopic(const std::string &topic_name) { return ros2_cli::isHiddenTopic(topic_name); }
...
std::uint8_t Ros2CliManager::serviceCallRpcTimeout(std::uint8_t service_timeout_sec) {
