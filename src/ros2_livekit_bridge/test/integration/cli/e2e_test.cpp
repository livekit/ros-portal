...
TEST_F(BridgeTestE2E, ListsRemoteRosServicesOverRpc) {
  ...
  EXPECT_TRUE(contains(show_types_response->output,
                       std::string(kVisibleService) + " [ros2_livekit_bridge_msgs/srv/Ros2ServiceList]"));
  ...
  EXPECT_FALSE(contains(include_hidden_response->output, "[ros2_livekit_bridge_msgs/srv/Ros2ServiceList]"));
...
TEST_F(BridgeTestE2E, CallsRemoteRosServiceOverRpc) {
  ...
  const auto missing_service_response = callServiceCallService(robotANode(), identityB(), "/bridge/missing_set_bool",
...
TEST_F(BridgeTestE2E, PublishesToRemoteRosTopicOverRpc) {
  ...
  ASSERT_NE(response, nullptr);
  EXPECT_TRUE(response->success) << response->err_msg;
  EXPECT_TRUE(delivered) << "Remote bridge did not deliver the published payload";
