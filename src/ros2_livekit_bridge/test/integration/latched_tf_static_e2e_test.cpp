...
// bridges join one LiveKit room on isolated DDS domains. Graph A publishes a
// latched (TRANSIENT_LOCAL) message BEFORE bridge B connects; the test asserts
...
// publisher, and bridge-to-bridge arrival + forwarding.
//
...
protected:
  ...
  std::shared_ptr<Ros2LiveKitBridge> makeBridge(const rclcpp::Context::SharedPtr& context,
                                                const std::string& node_namespace, const std::string& token,
                                                const std::string& config_path) {
    if (!setEnv("LIVEKIT_TOKEN", token) || !setEnv("LIVEKIT_URL", url_)) {
      ADD_FAILURE() << "Failed to set LiveKit environment variables";
      return nullptr;
    }
    auto bridge = std::make_shared<Ros2LiveKitBridge>(makeBridgeOptions(context, node_namespace, config_path));
    if (!bridge->initialize()) {
      ADD_FAILURE() << "Bridge failed to initialize for " << node_namespace;
      return nullptr;
    }
    return bridge;
  }
...
TEST_F(LatchedTfStaticE2E, RepublishesLatchedStatePublishedBeforePeerJoined) {
  ...
  // --- Graph A: publish the latched state BEFORE bridge B exists. ---
