...
/// @brief Construct a ServiceCall from a node.
ServiceCall makeCaller(const std::shared_ptr<rclcpp::Node>& node) {
  return ServiceCall(node->get_node_base_interface(), node->get_node_graph_interface(), node->get_logger());
}
...
TEST_F(ServiceCallTest, CallsServiceAndReturnsYamlAndCdrResponse) {
...
TEST_F(ServiceCallTest, RendersNestedResponseFieldsWithAccumulatingIndent) {
...
TEST_F(ServiceCallTest, RejectsEmptyInterfaceType) {
...
TEST_F(ServiceCallTest, HandlesUnavailableServiceTimeout) {
...
TEST_F(ServiceCallTest, DropsLateTimedOutResponseBeforeLaterCall) {
...
TEST_F(ServiceCallTest, RejectsEmptyRequestPayload) {
...
TEST_F(ServiceCallTest, RejectsUnknownServiceTypeWithoutThrowing) {
...
TEST_F(ServiceCallTest, RejectsServiceClientCacheLimit) {
