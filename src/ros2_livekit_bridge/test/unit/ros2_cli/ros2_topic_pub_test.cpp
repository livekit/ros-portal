...
protected:
  ...
  ros2_cli::TopicPublisher makePublisher(
    ros2_cli::TopicPublishAllowedFn allowed = {}) const
  {
    return ros2_cli::TopicPublisher(
...
TEST_F(Ros2TopicPubTest, PublishesYamlPayloadToTypedSubscriber)
{
  ...
  auto response =
    publisher.publish(makePublishOptions(topic, "std_msgs/msg/String", "{data: first}"));
  EXPECT_TRUE(response.success) << response.err_msg;
  ASSERT_TRUE(spinUntil([&]() {return node_->count_publishers(topic) == 1U;}));

  received.reset();
  response =
    publisher.publish(makePublishOptions(topic, "std_msgs/msg/String", "{data: second}"));
  EXPECT_TRUE(response.success) << response.err_msg;
...
TEST_F(Ros2TopicPubTest, ResolvesRelativeTopicNames)
{
  ...
  node_ = std::make_shared<rclcpp::Node>(
    "ros2_topic_pub_namespaced_unit_test", "/robot");
  ...
  auto response = publisher.publish(
    makePublishOptions(request_topic, "std_msgs/msg/String", "{data: first}"));
  EXPECT_TRUE(response.success) << response.err_msg;
  ASSERT_TRUE(
    spinUntil([&]() {return node_->count_publishers(resolved_topic) == 1U;}));

  received.reset();
  response = publisher.publish(
    makePublishOptions(request_topic, "std_msgs/msg/String", "{data: second}"));
  EXPECT_TRUE(response.success) << response.err_msg;
...
TEST_F(Ros2TopicPubTest, RejectsDeniedTopicBeforeCreatingPublisher)
{
  ...
  EXPECT_FALSE(spinUntil(
      [&]() {return node_->count_publishers(topic) != 0U;},
...
TEST_F(Ros2TopicPubTest, EnforcesPublisherCacheLimit)
{
  ...
  for (std::size_t i = 0; i < ros2_cli::kMaxCachedTopicPublishers; ++i) {
    const auto response = publisher.publish(
      makePublishOptions(
        "/topic_pub/cache_" + std::to_string(i),
        "std_msgs/msg/String",
        "{data: value}"));
