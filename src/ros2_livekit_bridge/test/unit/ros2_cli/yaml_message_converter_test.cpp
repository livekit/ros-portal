  ...
  const std::string & payload)
{
  std::string error;
  auto serialized =
    ros2_cli::serializedMessageFromYaml(interface_type, payload, error);
...
TEST(YamlMessageTest, SerializesBoundedString)
{
  const auto message = deserialize<test_msgs::msg::Strings>(
    serialize("test_msgs/msg/Strings", "{bounded_string_value: within bound}"));
...
TEST(YamlMessageTest, RejectsBoundedSequenceOverflow)
{
  ...
  expectFailure(
    "test_msgs/msg/BoundedSequences", "{int32_values: [1, 2, 3, 4]}");
}
...
YAML::Node node(const std::string & text)
{
...
TEST(YamlScalarTest, CheckedWCharAcceptsCharacterOrCode)
{
  ...
  EXPECT_EQ(checkedWChar(node("300"), "f", error),
    static_cast<char16_t>(300));
}
