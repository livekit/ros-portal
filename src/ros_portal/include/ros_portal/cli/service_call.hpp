...
public:
  ...
  ServiceCall(rclcpp::node_interfaces::NodeBaseInterface::SharedPtr base,
  ...
  ServiceCall(const ServiceCall&) = delete;
  ServiceCall& operator=(const ServiceCall&) = delete;

  /// @brief Call one ROS service using runtime service type support.
  ///
  /// Resolves the service name in ROS Portal node context, serializes the
...
private:
  ...
  std::optional<ServiceCallSrv::Response> takeResponse(ServiceClient& client, std::int64_t sequence_number);
