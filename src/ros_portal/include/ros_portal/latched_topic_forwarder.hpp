...
/// @brief Forwards latched ROS topics (e.g. `/tf_static`) between ROS Portal nodes over
...
///    ROS Portal started), stores the distinct messages it sees, and a background
///    worker pushes them to every peer ROS Portal node until each acknowledges over RPC,
///    re-pushing only when new state arrives or a peer rejoins. Peers that keep
///    failing are dropped after a cap so non-ROS Portal participants are not
///    hammered forever.
///  - Inbound: an RPC handler republishes received messages on a
///    TRANSIENT_LOCAL publisher so ROS subscribers that start after ROS Portal
...
public:
  ...
  /// @brief Forwarding configuration derived from ROS Portal config, plus
