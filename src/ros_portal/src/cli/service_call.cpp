...
ServiceCall::ServiceCall(rclcpp::node_interfaces::NodeBaseInterface::SharedPtr base,
  ...
  if (!base_ || !graph_) {
    throw std::invalid_argument("ServiceCall requires node base and graph interfaces");
  }
}
...
ServiceCallSrv::Response ServiceCall::call(const ServiceCallOptions& options) {
  ...
  try {
    ...
    return makeCliResponse<ServiceCallSrv::Response>(false, error.what());
  }
  ...
  if (auto resolved_client = getClient(resolved_service, msg_type, client_error)) {
    ...
    return makeCliResponse<ServiceCallSrv::Response>(false,
                                                     std::string("failed to create service client: ") + client_error);
  }
  ...
  if (!introspection::populateMessageFromYaml(options.msg_type + "_Request", options.payload, request_message.data(),
                                              yaml_error)) {
    return makeCliResponse<ServiceCallSrv::Response>(false, "failed to build service request: " + yaml_error);
  }
  ...
  if (send_ret != RCL_RET_OK) {
    ...
    rcl_reset_error();
    return makeCliResponse<ServiceCallSrv::Response>(false, "failed to send service request: " + message);
  }
...
std::optional<ServiceCallSrv::Response> ServiceCall::takeResponse(ServiceClient& client, std::int64_t sequence_number) {
  ...
  while (attempt_count < kMaxStaleResponseDrains) {
    ...
    try {
      ...
      return makeCliResponse<ServiceCallSrv::Response>(true, "", *output);
    } catch (const std::exception& error) {
      return makeCliResponse<ServiceCallSrv::Response>(
