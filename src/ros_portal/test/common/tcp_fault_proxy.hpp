...
private:
  ...
  public:
    ...
    void join() {
      if (client_to_upstream_.joinable()) {
        client_to_upstream_.join();
      }
      if (upstream_to_client_.joinable()) {
        upstream_to_client_.join();
      }
    }
  ...
  private:
    void pump(int source_fd, int destination_fd) {
      ...
      while (open_.load() && owner_.running_.load()) {
        if (!owner_.waitUntilResumed(open_)) {
          break;
        }
  ...
  int connectUpstream() const {
    ...
    if (::getaddrinfo(upstream_host_.c_str(), port.c_str(), &hints, &addresses) != 0) {
      return -1;
    }
    ...
    for (auto* address = addresses; address != nullptr; address = address->ai_next) {
      ...
      if (::connect(upstream_fd, address->ai_addr, address->ai_addrlen) == 0) {
        break;
      }
