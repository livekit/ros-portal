...
TEST_F(RosPortalTestE2E, ListsRemoteRosTopicsOverRpc) {
  initializeRuntime(kBidirectionalTopic);

  ASSERT_TRUE(waitFor([&]() { return publisherB()->get_subscription_count() > 0; }, kGraphTimeout))
      << "ROS Portal B did not subscribe to " << kBidirectionalTopic;

  constexpr const char* kHiddenTopic = "/_hidden_topic";
  auto hidden_publisher = robotBNode()->create_publisher<std_msgs::msg::String>(kHiddenTopic, 10);
  ASSERT_TRUE(waitFor([&]() { return topicExists(*robotBNode(), kHiddenTopic); }, kGraphTimeout))
      << "Hidden topic did not appear in ROS Portal B graph";
  ...
  const auto show_types_response = callTopicListService(robotANode(), identityB(), show_types_options);
  ASSERT_NE(show_types_response, nullptr);
  EXPECT_TRUE(show_types_response->success) << show_types_response->err_msg;
  EXPECT_TRUE(contains(show_types_response->output, std::string(kBidirectionalTopic) + " [std_msgs/msg/String]"));
  ...
  const auto include_hidden_response = callTopicListService(robotANode(), identityB(), include_hidden_options);
  ASSERT_NE(include_hidden_response, nullptr);
  EXPECT_TRUE(include_hidden_response->success) << include_hidden_response->err_msg;
  ...
  const auto verbose_response = callTopicListService(robotANode(), identityB(), verbose_options);
  ...
  EXPECT_TRUE(contains(verbose_response->output, " * /rosout [rcl_interfaces/msg/Log] 2 publishers\n"));
  EXPECT_TRUE(contains(verbose_response->output,
                       " * " + std::string(kBidirectionalTopic) + " [std_msgs/msg/String] 1 subscriber\n"));
  EXPECT_TRUE(contains(verbose_response->output, std::string(kBidirectionalTopic) + " [std_msgs/msg/String]"));
  ...
  const auto count_topics_response = callTopicListService(robotANode(), identityB(), count_topics_options);
  ASSERT_NE(count_topics_response, nullptr);
  EXPECT_TRUE(count_topics_response->success) << count_topics_response->err_msg;
  EXPECT_EQ(count_topics_response->output, std::to_string(lineCount(response->output)) + "\n");
  ...
  const auto missing_response = callTopicListService(robotANode(), "missing-livekit-participant");
...
TEST_F(RosPortalTestE2E, ListsRemoteRosServicesOverRpc) {
  initializeRuntime(kBidirectionalTopic);

  constexpr const char* kVisibleService = "/ros_portal/listable_service";
  auto visible_service = robotBNode()->create_service<ServiceListSrv>(
      kVisibleService, [](const std::shared_ptr<ServiceListSrv::Request>,
                          std::shared_ptr<ServiceListSrv::Response> response) { response->success = true; });
  ASSERT_NE(visible_service, nullptr);
  ASSERT_TRUE(waitFor([&]() { return serviceExists(*robotBNode(), kVisibleService); }, kGraphTimeout))
      << "Visible service did not appear in ROS Portal B graph";

  constexpr const char* kHiddenService = "/_hidden_service";
  auto hidden_service = robotBNode()->create_service<ServiceListSrv>(
      kHiddenService, [](const std::shared_ptr<ServiceListSrv::Request>,
                         std::shared_ptr<ServiceListSrv::Response> response) { response->success = true; });
  ASSERT_NE(hidden_service, nullptr);
  ASSERT_TRUE(waitFor([&]() { return serviceExists(*robotBNode(), kHiddenService); }, kGraphTimeout))
      << "Hidden service did not appear in ROS Portal B graph";
  ...
  show_types_options.show_types = true;
  const auto show_types_response = callServiceListService(robotANode(), identityB(), show_types_options);
  ASSERT_NE(show_types_response, nullptr);
  EXPECT_TRUE(show_types_response->success) << show_types_response->err_msg;
  EXPECT_TRUE(
      contains(show_types_response->output, std::string(kVisibleService) + " [ros_portal_msgs/srv/Ros2ServiceList]"));
  ...
  const auto include_hidden_response = callServiceListService(robotANode(), identityB(), include_hidden_options);
  ASSERT_NE(include_hidden_response, nullptr);
  EXPECT_TRUE(include_hidden_response->success) << include_hidden_response->err_msg;
  EXPECT_TRUE(contains(include_hidden_response->output, kHiddenService));
  EXPECT_FALSE(contains(include_hidden_response->output, "[ros_portal_msgs/srv/Ros2ServiceList]"));

  ServiceListServiceOptions count_services_options;
  count_services_options.count_services = true;
  const auto count_services_response = callServiceListService(robotANode(), identityB(), count_services_options);
  ASSERT_NE(count_services_response, nullptr);
  EXPECT_TRUE(count_services_response->success) << count_services_response->err_msg;
  EXPECT_EQ(count_services_response->output, std::to_string(lineCount(response->output)) + "\n");
  EXPECT_FALSE(contains(count_services_response->output, "/"));
  EXPECT_FALSE(contains(count_services_response->output, "["));

  const auto missing_response = callServiceListService(robotANode(), "missing-livekit-participant");
  ASSERT_NE(missing_response, nullptr);
  EXPECT_FALSE(missing_response->success);
  EXPECT_TRUE(contains(missing_response->err_msg, "missing-livekit-participant"));
}

TEST_F(RosPortalTestE2E, CallsRemoteRosServiceOverRpc) {
  initializeRuntime(kBidirectionalTopic);

  constexpr const char* kSetBoolService = "/ros_portal/set_bool";
  auto set_bool_service = robotBNode()->create_service<std_srvs::srv::SetBool>(
      kSetBoolService, [](const std_srvs::srv::SetBool::Request::SharedPtr request,
                          std_srvs::srv::SetBool::Response::SharedPtr response) {
        response->success = request->data;
        response->message = request->data ? "enabled" : "disabled";
      });
  ASSERT_NE(set_bool_service, nullptr);
  ASSERT_TRUE(waitFor([&]() { return serviceExists(*robotBNode(), kSetBoolService); }, kGraphTimeout))
      << "SetBool service did not appear in ROS Portal B graph";

  const auto response =
      callServiceCallService(robotANode(), identityB(), kSetBoolService, "std_srvs/srv/SetBool", "{data: true}");
  ASSERT_NE(response, nullptr);
  EXPECT_TRUE(response->success) << response->err_msg;
  EXPECT_TRUE(contains(response->output, "success: true"));
  EXPECT_TRUE(contains(response->output, "message: enabled"));

  const auto missing_type_response =
      callServiceCallService(robotANode(), identityB(), kSetBoolService, "", "{data: false}");
  ASSERT_NE(missing_type_response, nullptr);
  EXPECT_FALSE(missing_type_response->success);
  EXPECT_TRUE(contains(missing_type_response->err_msg, "msg_type must be non-empty"));

  ServiceCallServiceOptions timeout_options;
  timeout_options.timeout_sec = 1;
  const auto missing_service_response =
      callServiceCallService(robotANode(), identityB(), "/ros_portal/missing_set_bool", "std_srvs/srv/SetBool",
                             "{data: true}", timeout_options);
  ASSERT_NE(missing_service_response, nullptr);
  EXPECT_FALSE(missing_service_response->success);
  EXPECT_TRUE(contains(missing_service_response->err_msg, "Service call timed out"));

  const auto missing_participant_response = callServiceCallService(
      robotANode(), "missing-livekit-participant", kSetBoolService, "std_srvs/srv/SetBool", "{data: true}");
  ASSERT_NE(missing_participant_response, nullptr);
  EXPECT_FALSE(missing_participant_response->success);
  EXPECT_TRUE(contains(missing_participant_response->err_msg, "missing-livekit-participant"));
}

TEST_F(RosPortalTestE2E, ShowsRemoteRosInterfacesOverRpc) {
  initializeRuntime(kBidirectionalTopic);

  constexpr const char* kInterfaceType = "std_msgs/msg/Header";
  const auto response = callInterfaceShowService(robotANode(), identityB(), kInterfaceType);
  ASSERT_NE(response, nullptr);
  EXPECT_TRUE(response->success) << response->err_msg;
  EXPECT_TRUE(contains(response->output, "# Standard metadata for higher-level stamped data types."));
  EXPECT_TRUE(contains(response->output, "builtin_interfaces/Time stamp"));
  EXPECT_TRUE(contains(response->output, "\tint32 sec"));
  EXPECT_FALSE(contains(response->output, "# This message communicates ROS Time defined here:"));

  InterfaceShowServiceOptions all_comments_options;
  all_comments_options.all_comments = true;
  const auto all_comments_response =
      callInterfaceShowService(robotANode(), identityB(), kInterfaceType, all_comments_options);
  ASSERT_NE(all_comments_response, nullptr);
  EXPECT_TRUE(all_comments_response->success) << all_comments_response->err_msg;
  EXPECT_TRUE(contains(all_comments_response->output, "# This message communicates ROS Time defined here:"));

  InterfaceShowServiceOptions no_comments_options;
  no_comments_options.no_comments = true;
  const auto no_comments_response =
      callInterfaceShowService(robotANode(), identityB(), kInterfaceType, no_comments_options);
  ...
  const auto invalid_type_response = callInterfaceShowService(robotANode(), identityB(), "missing_pkg/msg/Thing");
  ...
  const auto missing_response = callInterfaceShowService(robotANode(), "missing-livekit-participant", kInterfaceType);
  ASSERT_NE(missing_response, nullptr);
  EXPECT_FALSE(missing_response->success);
  EXPECT_TRUE(contains(missing_response->err_msg, "missing-livekit-participant"));
}

TEST_F(RosPortalTestE2E, PublishesToRemoteRosTopicOverRpc) {
  ...
  constexpr const char* kPubPattern = "/ros_portal/.*";
  initializeRuntime(kPubPattern, kPubPattern, kBidirectionalTopic, kBidirectionalTopic);

  constexpr const char* kPubTopic = "/ros_portal/cmd";
  ...
  auto subscription = robotBNode()->create_subscription<std_msgs::msg::String>(
      kPubTopic, 10, [&](const std_msgs::msg::String::ConstSharedPtr message) {
  ...
  ASSERT_TRUE(waitFor([&]() { return topicExists(*robotBNode(), kPubTopic); }, kGraphTimeout))
      << "Subscriber topic did not appear in ROS Portal B graph";
  ...
  TopicPubSrv::Response::SharedPtr response;
  const bool delivered = waitFor(
      [&]() {
        response = callTopicPubService(robotANode(), identityB(), kPubTopic, kStringType,
  ...
  // The bidirectional topic is already in ROS Portal B's graph as a String, so a
  // mismatched requested type must be rejected against the known graph type.
  const auto type_mismatch_response =
      callTopicPubService(robotANode(), identityB(), kBidirectionalTopic, "std_msgs/msg/Int32", "{data: 1}");
