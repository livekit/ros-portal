...
TopicListSrv::Request makeRequest() {
...
ServiceListSrv::Request makeServiceListRequest() {
...
ServiceCallSrv::Request makeServiceCallRequest(std::string msg_type = "std_srvs/srv/SetBool") {
...
TopicPubSrv::Request makeTopicPubRequest() {
...
InterfaceShowSrv::Request makeInterfaceShowRequest() {
...
TEST(JsonConvertersTest, BuildsCliResponse) {
  const auto error_response = makeCliResponse<TopicListSrv::Response>(false, "boom", "");
  EXPECT_FALSE(error_response.success);
  EXPECT_EQ(error_response.err_msg, "boom");
  EXPECT_EQ(error_response.output, "");

  const auto ok_response = makeCliResponse<ServiceCallSrv::Response>(true, "", "success: true\n");
...
TEST(JsonConvertersTest, ParsesResponsePayload) {
  std::string error;
  const auto response = cliResponseFromJson<ServiceCallSrv::Response>(
...
TEST(JsonConvertersTest, MalformedPayloadsFail) {
  ...
  EXPECT_FALSE(cliResponseFromJson<TopicListSrv::Response>("not-json", error).has_value());
  EXPECT_FALSE(cliResponseFromJson<TopicListSrv::Response>(R"({"success":true})", error).has_value());
}
