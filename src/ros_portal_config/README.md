...
- `schema/ros_portal_config.schema.json`: source of truth for the YAML
  config structure and the generated C++ structs used by ROS Portal.
...
- `scripts/templates/config_parser.hpp.j2`: Jinja2 template for the public C++
  API consumed by ROS Portal, including `RosPortalConfig`, nested config structs,
...
- `include/ros_portal_config/config/error.hpp`: public exception type
...
- `generated/config_parser/include/ros_portal_config/config/config_parser.hpp`
...
used by ROS Portal and the parser validation code are regenerated from that
...
- building readable field paths like `$.ros_portal.topics`
