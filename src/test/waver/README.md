...
| `navigation/` | `waver_navigation` | SLAM Toolbox, Nav2, and explore_lite launch/config. |
...
- `explore` (`false`): launch autonomous frontier exploration; requires `nav2:=true`.
...
```bash
ros2 launch waver_bringup waver.launch.xml sim:=true

# or only the Gazebo entrypoint:
ros2 launch waver_simulation gazebo.launch.xml
```

## LiveKit Bridge

`waver_livekit_local.launch.py` brings up the WAVE ROVER stack together with the
local LiveKit bridge. It defaults to `waver_bringup/config/livekit.yaml`.

```bash
ros2 launch waver_bringup waver_livekit_local.launch.py \
  sim:=true \
  identity:=robot
```

For cloud credentials, launch `ros2_livekit_bridge` directly with the config in
`waver_bringup/config/livekit.yaml`.
